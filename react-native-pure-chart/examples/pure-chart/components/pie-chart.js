  ...
  initData(data) {
    ...
    if (data[0].color) {
      for (let i = 0; i < data.length; i++) {
        if (data[i].value > 0 && data[i].label) {
    ...
    //console.log('piePos', piePos, oldPiePos, pieSize, labels, colors, angles)
    ...
    */
    this.setState({
      labels: labels,
      colors: colors,
      pieSize: pieSize.filter(io => !isNaN(io)),
      piePos: piePos,
      selectedIndex: -1,
      pieIndex: pieIndex,
      angles: angles.filter(io => !isNaN(io)),
      dataSum: sum,
      oldPiePos: oldPiePos
    }, () => {
      this.forceUpdate();
...
const styles = StyleSheet.create({
  ...
  tooltipTitle: { fontSize: 16, fontWeight: 'bold', color: '#484545' },
