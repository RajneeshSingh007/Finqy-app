...
export default class CommonScreen extends React.PureComponent {
  ...
  header() {
    const {
      ...
      showProfile = false,
      leftIcon,
      imaguri = {uri: Pref.profileDefaultPic},
    } = this.props;
    return (
      <View styleName="horizontal" style={styles.headerBar}>
      ...
      <SafeAreaView style={styles.mainContainer} forceInset={{top: 'never'}}>
        <Screen style={styles.mainContainer}>
          ...
          {enabelWithScroll ? (
            ...
            <View style={{flex: 1}}>
              <View style={{flex: headerDis}}>{header}</View>
              <View style={{flex: bodyDis}}>{body}</View>
            </View>
          )}
