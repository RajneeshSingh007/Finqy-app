...
export default class Blogs extends React.PureComponent {
    ...
    fetchData = () => {
        this.setState({ loading: true, });
        Helper.networkHelperToken(Pref.BlogsUrl, Pref.methodGet, this.state.token, result => {
            ...
            if (res_type === `success`) {
                ...
                const catlist = Lodash.map(data, io => {
                    const { category} = io;
                    const find = Lodash.find(categoryList, ui => ui.name === category);
                    if (find === undefined) {
    ...
    renderItems(item, index) {
        ...
        return (
        <TouchableWithoutFeedback onPress={() => NavigationActions.navigate(`BlogDetails`, { item: item })}>
                <View styleName='vertical' style={{
                    ...
                    <Card elevation={2} style={{ borderRadius: 8 }}>
                        ...
                        />
                        <View style={{
                            marginVertical: 16, marginHorizontal: 12
                        }}>
                            <Title styleName='v-start h-start' numberOfLines={1} style={{
                                fontSize: 15, fontFamily: 'Rubik', letterSpacing: 1, color: '#292929', fontWeight: '700',
                            }}>{`${item.title}`}</Title>
                            <Subtitle styleName='v-start h-start' numberOfLines={3} style={{
                                fontSize: 14, fontFamily: 'Rubik', letterSpacing: 0, color: '#767676', fontWeight: '400',
                            }}>{`${item.desc}`}</Subtitle>
                        </View>
                    </Card>
                </View>
