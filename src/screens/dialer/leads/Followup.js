...
export default class Followup extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      dataList: [],
        ...
        (result) => {
          ...
          if (status) {
            if (data.length > 0) {
              ...
              widthArr = [60, 120, 110, 110, 80, 120, 140,140,60];
      ...
      if (start >= 0) {
        for (let i = start; i < end; i++) {
          ...
          if (Helper.nullCheck(item) === false) {
            ...
            rowData.push(item.followup_datetime === null || item.followup_datetime === '' ? '' : item.followup_datetime);
            ...
            {this.state.loading ? (
              ...
              />
                  ...
                  <View style={styles.emptycont}>
                    <ListError subtitle={ 'No Follow-up Records Found...'} />
                  </View>
                )}
            {this.state.dataList.length > 0 ? (
              <>
