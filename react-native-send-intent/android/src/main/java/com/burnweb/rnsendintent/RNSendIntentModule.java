...
public class RNSendIntentModule extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void whatsappPhone(ReadableMap options,final Promise promise){
        boolean checkPermissions = PermissionChecker.checkSelfPermission(this.reactContext, Manifest.permission.READ_CONTACTS) == PermissionChecker.PERMISSION_GRANTED && PermissionChecker.checkSelfPermission(this.reactContext, Manifest.permission.WRITE_CONTACTS) == PermissionChecker.PERMISSION_GRANTED;
        if(checkPermissions) {
    ...
    private String getContactName(final String phoneNumber) {
        ...
        String contactName="";
        Cursor cursor=this.reactContext.getContentResolver().query(uri,projection,null,null,null);
        if (cursor != null) {
